 ...
 */
export default function FscPanel({ variant = 'full', invert = false }: FscPanelProps) {
  ...
  if (variant === 'brief') {
    return (
      <div className={`border ${invert ? 'border-line-invert' : 'border-line-strong'} p-6 lg:p-8`}>
      ...
      <div className="lg:col-span-5">
        ...
        <dl className="mt-10 rise">
          ...
          <div className={`flex items-baseline justify-between gap-6 py-4 border-y ${line}`}>
            <dt className={`text-manifest-sm ${lead}`}>Certificate code</dt>
            <dd className={`text-body numeral text-right ${fsc.code ? (invert ? 'text-paper' : 'text-ink') : lead}`}>
              {/* Stated as pending rather than invented: an FSC claim without the
                  holder's own code is one the buyer cannot verify. */}
